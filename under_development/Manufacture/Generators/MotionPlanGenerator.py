...
class MotionPlanGenerator(Generator):
    ...
    def get_motion_plan(self, goal_transform_values, check_collision=True, dof=False):
        ...
        # with self.env:
        if not dof:
            h = misc.DrawAxes(self.simulator.env, goal_transform_values)
            if Config.IK_SOLVER == 'ik_fast':
                iksolution = self.simulator.get_ik_solutions(robot, goal_transform_values, check_collisions=True)
            elif Config.IK_SOLVER == 'trac_ik':
                iksolution = self.simulator.get_trac_ik_solutions(robot, goal_transform_values, check_collisions=True)
        else:
