...
import openravepy
from Solution.RobotAction import RobotAction
from Solution.LowLevelProblemSolution import LowLevelProblemSolution
from States.OpenRaveLowLevelState import OpenRaveLowLevelState
from IKSolvers.OpenRaveIKSolver import OpenRaveIKSolver

class OpenRavePlannerV2(Planner):
    ...
    def solve_object_pose(self, state, ll_problem):
        ...
        iksols = OpenRaveIKSolver(state).get_ik_solutions(ll_problem.generated_value, do_check_collisions=False)
        trajobj = None
        new_ll_state = None
        if len(iksols) > 0:
            ik_index = 0
