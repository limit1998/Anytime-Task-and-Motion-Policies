(define (domain keva)
  ...
  (:action pickUp_plank_from_region
  		 ...
  		 :precondition (and
  		    (handempty)
            (clearPlank ?p)
            (or (human_place ?p ?loc)(inBuffer ?p))
  ...
  (:action human_place
    ...
    :effect (and
        (probabilistic
            ;0.0 (and (human_place ?p location1)(not (free location1)(clearPlank ?p)))
            1.0 (and (human_place ?p location2)(not (free location2))(clearPlank ?p))
        )
    )
  )
