...
class LLActionSpecV2(object):
    ...
    def __init__(self, name, precondition, effect, updates_ll_state = False,execution_sequence=None,attach=None,hl_args = None,ll_args = None):
        self.name = name
    ...
    def __deepcopy__(self, memodict={}):
        temp = LLActionSpecV2(copy.deepcopy(self.name), copy.deepcopy(self.precondition), copy.deepcopy(self.effect),execution_sequence=copy.deepcopy(self.exec_sequence),attach=copy.deepcopy(self.attach),hl_args=copy.deepcopy(self.hl_args))
    ...
    def generator(self,low_level_state, backtrack_on_failure=True,start_time = None):
        ...
        while True:
            if self.generated_values is None:
                generated_values = {"attach" : self.attach}
                # generated_values.update(high_level_arguments)
            else:
            ...
            while i < len(self.predicates):
                ...
                try:
                    ...
                    while j < len(arguments):
                        ...
                        try:
                            if arg_generator is not None:
                                ...
                                print "Using Arg: " + argument.name +" alias: "+str(argument.name_alias)+" Val: "+str(generated_values.get(argument.name_alias))+ " Predicate: " + predicate.name + " Action: " + self.name
                    ...
                    evaluation, fail_strings = predicate(low_level_state=low_level_state, arg_map = generated_values)
                    if not evaluation:
