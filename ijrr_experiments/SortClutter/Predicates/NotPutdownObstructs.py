...
class NotPutdownObstructs(Predicate):
    ...
    def __call__(self,**kwargs):
        ...
        resulting_env = ll_state.simulator.set_robot_active_dof_values_to_waypoint_values(ll_state, trajobj.GetWaypoint(
            waypoints_count - 1))
        colliding_objects = ll_state.simulator.get_colliding_objects(resulting_env)
        for o in colliding_objects:
            if o != arg_map["obj"] and o != arg_map["table"]:
        ...
        if len(colliding_object_names) > 0:
            for o in colliding_object_names:
                fail_strings.append("(putdownobstructs {} {} {} {})".format(arg_map["trajectory"],o,arg_map["obj"],arg_map["table"]))
