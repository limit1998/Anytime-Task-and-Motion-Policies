...
class PutDownPoseGenerator(Generator):
    def __init__(self, ll_state=None, known_argument_values=None):
        ...
        self.table_name = self.known_argument_values["table"]
    ...
    def _compute_put_down_pose_list(self):
        ...
        while i < 3 and j < 50:
            ...
            if self.table_name == "main_table":
                obj_x, obj_y = obj_y,obj_x
            ...
            if self.table_name == "green_table":
                r_t = matrixFromAxisAngle([0,0,math.pi])
                obj_transform_wrt_table = obj_transform_wrt_table.dot(r_t)
            elif self.table_name == "main_table":
                # r_t = matrixFromAxisAngle([0, 0, -math.pi])
                # obj_transform_wrt_table = obj_transform_wrt_table.dot(r_t)
                pass
